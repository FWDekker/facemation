...
class CaptionStage(Stage):
    ...
    def __init__(self, layer_in: int, layer_out: int, cache_dir: str, caption_generator: CaptionGenerator):
        """
        Constructs a new `CaptionStage`.

        :param layer_in: the layer to adjust caption dimensions to
    ...
    def process(self, frames: List[Frame]) -> List[Frame]:
        ...
        for frame in tqdm(frames, desc="Adding captions", file=sys.stdout):
            width, height = ImageLoader.get_dims(frame["layers"][self.layer_in])
            caption = str(self.caption_generator(Path(frame["path"]).name))

            caption_hash = Hasher.hash_string(f"{width}-{height}-{caption}")
            if self.captioned_cache.has(caption_hash):
                frame["layers"][self.layer_out] = self.captioned_cache.path(caption_hash)
                continue
            ...
            frame["layers"][self.layer_out] = self.captioned_cache.cache(img, caption_hash)
